...
export async function askQuestions() {
  prompts.intro("Welcome to the quality standards initializer!");
  try {
    const language = await answer(prompts.select({
    ...
    const runtime = await answer(prompts.select({
    ...
    const projectType = await answer(prompts.select({
    ...
    const wantsSemgrep = await answer(prompts.confirm({
      message: "Configure security checks with Semgrep?",
      initialValue: false,
    }));
    const wantsLicense = await answer(prompts.confirm({
    ...
    let copyrightHolder = "";
    if (wantsLicense) {
      licenseType = await answer(prompts.select({
      ...
      copyrightHolder = await answer(prompts.text({
    ...
    const agentTargets = await answer(prompts.multiselect({
    ...
    const useSemgrep = wantsSemgrep && await prepareSemgrep(() =>
